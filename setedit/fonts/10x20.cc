...
// 4 son los bytes por línea y 10 el ancho ¿y el 20?
// 0x56A4 son los 4252 offsets de cada font de 80 bytes (17008 bytes=>0x4270)
// Después vienen los 4252 shapes
// La 1er letra 0x9924 y son 4*4252*20=340160 bytes => hasta 0x5C9E4
// Ahí dice 0x102, luego 4252 0x61cf5-0x5c9ea, serían otros 5 bytes por caracter:
// 0x61d04 son 2 bytes y van diciendo para cada unicode que símbolo lo resuelve.
// Si no hay símbolo pone 0xFFFF. Son unas 0x3100 entradas.
     ...
     if (uni>=0x3100)
       {
        if (uni==0xFFFD)
           ...
           printf("Código %d está fuera del rango\n",code);
       }
     else
       {
        int val=unicodes[uni];
