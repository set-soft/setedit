...
#if !defined(CLY_UseCrLf) && !defined(SEOSf_Linux)
/* When !CLY_UseCrLf is defined I use getline, but that's a GNU extension
   this code should be in compatlayer but TV is frozen right now. */
...
ssize_t getline_safe(char **_lineptr, size_t *_n, FILE *_stream, int limit);
ssize_t getstr(char **_lineptr, size_t *_n, FILE *_stream, char _terminator,
...
ssize_t getstr(char **lineptr, size_t *n, FILE *stream, char terminator,
...
ssize_t getline_safe(char **lineptr, size_t *n, FILE *stream, int limit)
{
 return getstr(lineptr,n,stream,'\n',0,limit);
}
#endif // !defined(CLY_UseCrLf) && !defined(SEOSf_Linux)
          ...
          if (crfound)
            {
             ...
             char *line=0;
             size_t lenLine=0;
             while ((len=getline(&line,&lenLine,f))!=-1)
               {
...
Boolean  TCEditor::staticShowMatchPairFly=False;
Boolean  TCEditor::staticShowMatchPairNow=False;
...
Boolean  TCEditor::staticTabIndents=True;
